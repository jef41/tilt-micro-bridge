...
class TiltStatus(JsonSerialize):
    ...
    def __init__(
            ...
            try:
                self.temp_fahrenheit = round(
                    TiltStatus.apply_cal(
                        uncal_temp_fahrenheit, config.get_temp_offsets(self.colour)
                    ),
                    2,
                )
            except Exception as e:
                raise type(e)(
                    f"TiltStatus: Temperature calibration is invalid, ignoring: {e}: "
                ) from e
                # TODO create logger here?
                self.temp_fahrenheit = uncal_temp_fahrenheit
            # print(f"***  cal vals {colour} {vals}")
            self.gravity = TiltStatus.apply_cal(
                uncal_SG, config.get_gravity_offsets(self.colour)
            )
        ...
        self.alcohol_by_volume = TiltStatus.get_alcohol_by_volume(
            self.original_gravity, self.gravity
        )
        self.apparent_attenuation = TiltStatus.get_apparent_attenuation(
            self.original_gravity, self.gravity
        )
        self.temp_valid = (
            config.temp_range_min < self.temp_fahrenheit
            and self.temp_fahrenheit < config.temp_range_max
        )
        self.gravity_valid = (
            config.gravity_range_min < self.gravity
            and self.gravity < config.gravity_range_max
        )
    ...
    def get_degrees_plato(gravity):
        return round(
            1111.14 * gravity - 630.272 * gravity**2 + 135.997 * gravity**3 - 616.868, 1
        )
    ...
    def get_alcohol_by_volume(original_gravity, current_gravity):
        ...
        alcohol_by_volume = (
            76.08 * (original_gravity - current_gravity) / (1.775 - original_gravity)
        ) * (current_gravity / 0.794)
