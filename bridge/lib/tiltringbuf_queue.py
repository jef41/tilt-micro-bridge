# tiltringbuf_queue.py Provides RingbufQueue class
...
class TiltRingbufQueue:  # MicroPython optimised
    ...
    async def put_struct(self, fmt, *data):  # Usage: await queue.put(item)
        while self.full():  # Queue full
            await self._evget.wait()  # May be >1 task waiting on ._evget
            # Task(s) waiting to get from queue, schedule first Task
        self.put_struct_nowait(fmt, *data)
